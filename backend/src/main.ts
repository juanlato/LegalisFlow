...
async function bootstrap() {
  ...
  const tenantsService = app.select(TenantsModule).get(TenantsService, { strict: true });
  ...
  app.enableCors({
    origin: async (origin, callback) => {
      ...
      const domainPattern = /^https?:\/\/(?:([^\.]+)\.)?midominio\.com(?:\.\w+)?(:\d+)?$/;
      ...
      const subdomain = matches[1] || 'www'; // Si no hay subdominio, usar 'www'     
        ...
        const tenant = await tenantsService.findActiveBySubdomainCors(subdomain);
