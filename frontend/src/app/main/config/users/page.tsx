...
export default function UsersPage() {
  ...
  const handleCreate = async (formData: any) => {
    setLoading(true);
    try {
      let user = await createUser(formData);
      await assignRole(user.id, formData.roleId);
      fetchUsers();
  ...
  const handleEdit = async (formData: any) => {
    setLoading(true);
    try {
      let user = await updateUser(modalState.data.id, formData);
      await assignRole(user.id, formData.roleId);
      fetchUsers();
      setModalState({ type: null });
    } catch (error) {
    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="space-x-2">
          <button onClick={() => setModalState({ type: 'filter' })} className="btn-filter">
            Filtrar
          </button>
